...
2. Net *Namespace*: Isola lo [[ISO-OSI|stack di rete]], il container vede le proprie interfacce di rete, [[Routing#Tabella di Routing IP|tabelle di routing]], [[Livello di Trasporto#Numero di Porta|porte]] e regole di [[Firewall]], indipendenti dall'host.
...
5. `IPC` *Namespace*: Isola i meccanismi di comunicazione tra processi, come code di messaggi, segmenti di memoria condivisa e [[13 - Semafori|semafori]].
...
- *CGroup* delle risorse di calcolo ([[La CPU|CPU]]): **Limitazione del tempo** di `CPU` e pinning su core specifici.
- *CGroup* della memoria ([[RAM]]): **Limiti rigidi sulla memoria fisica** e sulle swap. Se il gruppo di processi eccede il limite, interviene l'Out Of Memory Killer nel contesto del container.
